...
add_port() {
    ...
    ssh -o StrictHostKeyChecking=no -f -N -R 0.0.0.0:${random_port}:localhost:${local_port} 138.68.79.95 > /dev/null &
    ssh_pid=$!
...
remove_port() {
    ...
    pkill -f "ssh -o StrictHostKeyChecking=no -f -N -R 0.0.0.0:${random_port}:localhost:${local_port} 138.68.79.95" > /dev/null
...
list_ports() {
    ...
    while IFS= read -r line; do
        random_port=$(echo $line | cut -d':' -f1)
        local_port=$(echo $line | cut -d':' -f2)
        echo "Local port ${local_port} -> Public port ${random_port} ()"
    done < $PORTS_FILE
}
